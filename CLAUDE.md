...
- `src/pipeline/unwrap.ts` and `bake.ts` — spike (issue #10), off unless the address has `?bake=<size>`: xatlas texture coordinates for the table level and detail maps baked from the sculpt. See the spec for the no-go on baked normal maps.
