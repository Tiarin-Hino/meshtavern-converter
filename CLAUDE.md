...
- Spike #34 (a faster unwrap; results in the Phase 1 spec, the change itself is #57): `src/pipeline/parts.ts` (connected pieces, slabs), `unwrap-parts.ts` (unwrap per slab, in workers or as meshes of one atlas), `seams.ts` (seam length), `src/worker/islands.worker.ts` and `island-pool.ts`, `scripts/spike34/` (`dump-tables.mjs`, `sweep.mjs` in Node, `measure.mjs` in Chrome, into `out/spike34/`). Development only, behind `?unwrap=whole|cut<N>|multi<N>` (`&workers=`, `&chart=`); without it the normal path is untouched. #57 removes what lost.
