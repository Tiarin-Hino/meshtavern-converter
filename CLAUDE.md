...
- `src/viewer.ts` — three.js scene. `src/main.ts` — UI wiring and the `window.__mt` test hook (`state`, `loadDemo()`, `loadGenerated(n)`, `showLevel(i)`, `setCamera(azimuth, elevation, zoom)`, `setWireframe(on)`, `setLook(changes)`, `startStress(count, forcedLod?)`, `stopStress()`, `setUp(axis)`, `poolForStress(share)`, `clearStressPool()`; live figures on `state.perf`).
