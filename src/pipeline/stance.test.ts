...
import { restingFacet, restingPoints } from './stance';
...
const height = (points: Float64Array, i: number, up: Vec3): number =>
  points[i * 3]! * up[0] + points[i * 3 + 1]! * up[1] + points[i * 3 + 2]! * up[2];
...
describe('restingFacet', () => {
  ...
  it('is a supporting plane from whichever side the points are dropped', () => {
    ...
    for (const down of downs) {
      const { up, corners } = restingFacet(points, down);
      const touching = height(points, corners[0], up);
      for (let i = 0; i < points.length / 3; i++)
        expect(height(points, i, up)).toBeGreaterThan(touching - 0.05);
    }
  });
