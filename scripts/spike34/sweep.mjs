...
const { unwrap, unwrapperReady, MAX_CHART_COST } = await vite.ssrLoadModule('/src/pipeline/unwrap.ts');
const { connectedPieces, groupPieces, splitByGroup, cutIntoSlabs } = await vite.ssrLoadModule('/src/pipeline/parts.ts');
const { findIslands, packParts, sharedTexelsPerUnit } = await vite.ssrLoadModule('/src/pipeline/unwrap-parts.ts');
...
const names = [...new Set(readdirSync(TABLES).filter((f) => f.endsWith('.indices')).map((f) => f.slice(0, -8)))].filter((n) => n.includes(filter));
...
for (const name of names) {
  ...
  for (const variant of variantList.split(';')) {
    ...
    if (!split) {
      ...
      row.chartTypes = islands.reduce((sum, i) => sum.map((n, k) => n + i.chartTypes[k]), [0, 0, 0, 0, 0]);
    }
