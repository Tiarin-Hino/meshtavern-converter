...
- **Pass mark**: all 30 corpus minis upright by detection alone. The corpus index records the expected up direction per mini and `npm run corpus` reports every mismatch, the same mechanism #44 proposes for the creature size; the generated shapes of the CI baseline get the same check.
