...
- **The suggestion** (`size.ts`): the smallest footprint the base fits into, with 5 % tolerance; within one square, under 26 mm is Small. A mini without a base is suggested Medium (see below). The size names are the SRD 5.1's (CC-BY-4.0, attribution in `README.md`); the squares are ours.
...
- The Small threshold for 20 mm bases waits for the PM (PR #74). Accepted on the PR: the far-level shift, Medium without a base, generic corpus names, the index.
