...
5. **A converter other code can call (new).** `src/` splits into a library (pipeline, worker, a small typed API: convert a file with options, progress events, cancel, result with levels, texture and figures) and the page that uses it. The table application will import the library, not copy the page. No behaviour change.
