...
4. **Scale and base (#44).** Units guessed from the size (mm, inches, metres), shown, and correctable. An existing base is detected and measured. **PM decision, 2026-09-23: one grid square is 32 mm in mini space.** Tiny, Small and Medium creatures take 1×1 square; Large 2×2 (64 mm); Huge 3×3 (96 mm); Gargantuan 4×4 (128 mm). A base may be smaller than its footprint: the mini is centred in the footprint, not scaled to fill it. The page suggests the creature size from the measured base (the smallest footprint it fits into, with 5 % tolerance _(proposal)_); the user accepts it or picks another. Size names are the SRD 5.1 ones (CC-BY-4.0), shown with the footprint in squares so other systems fit. By default the mini keeps its measured size, with its origin at the centre of its base; the user can scale it to a base diameter they enter _(proposal)_. A base larger than the chosen footprint gets a warning and an offer to scale. Minis without a base can get a plain round one, 32, 50, 75 or 100 mm by size _(proposal)_. The result records size, footprint and base diameter for the table and the library API (#50). The viewer's grid, today one inch in `src/viewer.ts`, becomes the 32 mm square when this story is built, and the stress scene spaces its minis by it. Nothing is rescaled silently. Before 2026-09-23 this story offered scaling to a list of standard base sizes; the grid decision replaced that.
