...
11. **Figure stands upright (#72).** Agreed with the PM on 2026-09-23, from the open question in #44; in the order it sits between story 4 and story 10 _(the mechanics are proposals unless marked as a PM decision)_. The first corpus run found 7 of 30 minis not upright, all figure files without a base in print orientation: two quadrupeds, a mounted figure, the largest dragon, a swarm on its edge, two flyers. Today the base detector (`detectUpAxis`) finds nothing on them and falls back to the taller of Y-up and Z-up, which suits standing humanoids and nothing long and low; some of the seven are tilted, which the six-way selector cannot fix. The story: when no base is found, the candidate directions are scored by the signs of a resting creature (lowest points form a wide, level cluster; more volume above the middle than below; print convention only breaks ties); the mini is then set down on its lowest points so the resting plane is level within 2°; the user can turn it freely on the page and set it down again, the six-way select stays as the coarse step, nothing is turned silently; the orientation is recorded in the result as a rotation. Pass mark: all 30 corpus minis upright by detection alone, the expected up direction per mini in the corpus index and checked by `npm run corpus`, the generated shapes by the CI baseline; detection under 200 ms for the largest file. Story 4 places its plain base under the result and until then builds on today's fallback; story 10 uses it to find the feet. Out: minis on a base (the base detector keeps priority), facing direction, minis lying on purpose, a learned model.
